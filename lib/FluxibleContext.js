 ...
 */
FluxContext.prototype.executeAction = function executeAction(action, payload, done, scope) {
    var self = this;
    ...
    var executeActionPromise = new Promise(function executeActionPromise(resolve, reject) {
        ...
        setImmediate(function () {
            try {
                executeActionInternal(self, action, payload, resolve, reject, scope);
            } catch (err) {
                reject(err);
            }
        });
 ...
 * @private
 */
function executeActionInternal(context, action, payload, resolve, reject, scope) {
    ...
    var result;
    if(!scope){
        result = action(context.getActionContext(), payload, cb);    
    } else {
        console.log("SCOPE:", scope.config)
        result = action.call(scope, context.getActionContext(), payload, cb);
    }
...
FluxContext.prototype.getActionContext = function getActionContext() {
    ...
    var actionContext = {
        ...
        getDomain: this._dispatcher.getDomain.bind(this._dispatcher)
    };
